...
class Index extends AppadminbaseBlock implements AppadminbaseBlockInterface
{
    ...
    protected $_enableUrl;
    protected $_disableUrl;
    ...
    public function init()
    {
        ...
        $this->_enableUrl = CUrl::getUrl('catalog/productinfo/bulkenable');
        $this->_disableUrl = CUrl::getUrl('catalog/productinfo/bulkdisable');
        
        /*
         * edit data url
         */
        $this->_editUrl = CUrl::getUrl('catalog/productinfo/managerbatchedit');
    ...
    public function getTableTbodyHtml($data)
    {
        ...
        foreach ($data as $one) {
            ...
            //$str .= '<td>
			//			<a style="width:17px;" title="' . Yii::$service->page->translate->__('Edit')  . '" target="dialog" class="btnEdit" mask="true" drawable="true" width="1200" height="680" href="'.$this->_editUrl.'?edit_spu='.$one['spu'].'" ><i class="fa fa-pencil" style="font-size:16px;"></i></a>
			//			<a style="width:17px;" title="' . Yii::$service->page->translate->__('Delete')  . '" target="ajaxTodo" href="'.$this->_deleteUrl.'?'.$this->_primaryKey.'='.$one[$this->_primaryKey].'" class="btnDel"  csrfName="' .CRequest::getCsrfName(). '" csrfVal="' .CRequest::getCsrfValue(). '" ><i class="fa fa-trash-o" style="font-size:16px;"></i></a>
			//		</td>';
            $str .= '<td>
						<a style="width:17px;" title="' . Yii::$service->page->translate->__('Edit')  . '" target="dialog" class="btnEdit" mask="true" drawable="true" width="1200" height="680" href="'.$this->_editUrl.'?edit_spu='.$one['spu'].'" ><i class="fa fa-pencil" style="font-size:16px;"></i></a>
					</td>';
    ...
    public function getEditBar()
    {
        ...
        //            <li><a class="add"   href="'.$this->_batchInsertUrl.'"  target="dialog" height="680" width="1200" drawable="true" mask="true"><span>' . Yii::$service->page->translate->__('Add') . '</span></a></li>
        ...
        return '<ul class="toolBar">
              	<li><a csrfName="' .CRequest::getCsrfName(). '" csrfVal="' .CRequest::getCsrfValue(). '" title="' . Yii::$service->page->translate->__('Are you sure you want to put these products on the shelves') . '?" target="selectedTodo" rel="'.$this->_primaryKey.'s" postType="string" href="'.$this->_enableUrl.'" class="edit"><span>' . Yii::$service->page->translate->__('Bulk Enable') . '</span></a></li>
                <li><a csrfName="' .CRequest::getCsrfName(). '" csrfVal="' .CRequest::getCsrfValue(). '" title="' . Yii::$service->page->translate->__('Are you sure you want to take these products off the shelves') . '?" target="selectedTodo" rel="'.$this->_primaryKey.'s" postType="string" href="'.$this->_disableUrl.'" class="edit"><span>' . Yii::$service->page->translate->__('Bulk Disable') . '</span></a></li>
            </ul>';
    }
}
