...
//use fecbbc\app\appbdmin\modules\Catalog\block\productinfo\index\Attr;
use Yii;
...
class Manageredit extends AppbdminbaseBlockEdit implements AppbdminbaseBlockEditInterface
{
     ...
     */
    protected $_attrBlockName = '\fecbbc\app\appbdmin\modules\Catalog\block\productinfo\index\Attr';
    ...
    {
        $productPrimaryKey = $this->_service->getPrimaryKey();
        $v = Yii::$app->request->get($productPrimaryKey);
        if ($v && $this->_bdmin_user_id != $this->_one['bdmin_user_id']) {
            throw new InvalidCallException('you do not have role to operate it');
        }
    ...
    public function save()
    {
        ...
        if ($productId && !Yii::$service->product->isBdminHasEditRole($productId,  $this->_bdmin_user_id)) {
            echo  json_encode([
                'statusCode' => '300',
    ...
    public function delete()
    {
        ...
        if ($id = CRequest::param($this->_primaryKey)) {
            $ids = $id;
            // 验证权限
            if (!Yii::$service->product->isBdminHasEditRole($id,  $this->_bdmin_user_id)) {
                echo  json_encode([
                    'statusCode' => '300',
                    'message'    => 'you do not have role',
                ]);
            ...
            if (is_array($ids)) {
                foreach ($ids as $productId) {
                    // 验证权限
                    if (!Yii::$service->product->isBdminHasEditRole($productId,  $this->_bdmin_user_id)) {
                        echo  json_encode([
                            'statusCode' => '300',
                            'message'    => 'you do not have role',
                        ]);
        ...
        $this->_service->removeByProductIdsWithSameSpu($ids);
        $errors = Yii::$service->helper->errors->get();
        if (!$errors) {
            echo  json_encode([
                'statusCode' => '200',
