 ...
 * @since 1.0
 */
class Getproductcategory extends \yii\base\BaseObject
{
    public function getProductCategory()
    {
        $coupon_id = CRequest::param('coupon_id');

        $menuStr = $this->getMenuStr($coupon_id);
        echo json_encode([
            'menu'=>$menuStr,
    ...
    public function getMenuStr($coupon_id)
    {

        //$menu = $this->getMenArray($coupon_id);
        $menu = Yii::$service->category->getTreeArr();
       
        $category_ids = $this->getCategoryByCouponId($coupon_id);
        $str = '';
    ...
    // 得到產品的分類id
    public function getCategoryByCouponId($coupon_id)
    {
        return Yii::$service->coupon->getCategoryIdsByCouponId($coupon_id);
    }

    public function getMenu($coupon_id)
    {
        //设置在config设置的值
    ...
    //website
    public function getMenArray($coupon_id)
    {
        $this->getMenu($coupon_id);
        $root_catefory = '';
