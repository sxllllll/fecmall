...
namespace fecbbc\app\appbdmin\modules\Sales\block\orderaudit;
...
class Manager extends AppbdminbaseBlock implements AppbdminbaseBlockInterface
{
    ...
    public function init()
    {
         ...
         * edit data url
         */
        $this->_editUrl = CUrl::getUrl('sales/orderaudit/manageredit');
        /*
         * delete data url
         */
        $this->_auditAcceptUrl = CUrl::getUrl('sales/orderaudit/manageraccept');
        
        $this->_auditRefuseUrl = CUrl::getUrl('sales/orderaudit/managerrefuse');
    ...
    { 
        $orderStatus = [
            Yii::$service->order->payment_status_confirmed => Yii::$service->page->translate->__(Yii::$service->order->payment_status_confirmed),
            Yii::$service->order->payment_no_need_status_confirmed => Yii::$service->page->translate->__(Yii::$service->order->payment_no_need_status_confirmed),
            Yii::$service->order->status_audit_fail => Yii::$service->page->translate->__(Yii::$service->order->status_audit_fail),
    ...
    public function getEditBar()
    {
        return '<ul class="toolBar">
					<li><a csrfName="' .CRequest::getCsrfName(). '" csrfVal="' .CRequest::getCsrfValue(). '" title="' . Yii::$service->page->translate->__('Are you sure audit order accept in bulk') . '?" target="selectedTodo" rel="'.$this->_primaryKey.'s" postType="string" href="'.$this->_auditAcceptUrl.'" class="edit"><span>' . Yii::$service->page->translate->__('Bulk Audit Accept') . '</span></a></li>
                    <li><a csrfName="' .CRequest::getCsrfName(). '" csrfVal="' .CRequest::getCsrfValue(). '" title="' . Yii::$service->page->translate->__('Are you sure audit order refuse in bulk') . '?" target="selectedTodo" rel="'.$this->_primaryKey.'s" postType="string" href="'.$this->_auditRefuseUrl.'" class="edit"><span>' . Yii::$service->page->translate->__('Bulk Audit Refuse') . '</span></a></li>
				</ul>';
    }
