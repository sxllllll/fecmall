...
class Manager extends AppbdminbaseBlock implements AppbdminbaseBlockInterface
{
    ...
    { 
         ...
         * edit data url
         */
        $this->_editUrl = CUrl::getUrl('sales/orderdispatch/manageredit');
        /*
         * delete data url 
         */
        $this->_exportExcelUrl = CUrl::getUrl('sales/orderdispatch/managerexport');
        $this->_dispatchUrl = CUrl::getUrl('sales/orderdispatch/managerdispatch');
    ...
    { 
        $orderStatus = [
            ...
            Yii::$service->order->status_dispatched => Yii::$service->page->translate->__(Yii::$service->order->status_dispatched),
        ];
        $data = [
            [    // 字符串类型
                'type' => 'inputtext',
    ...
    public function getTableTbodyHtml($data)
    {
        ...
        foreach ($data as $one) {
            ...
            $str .= '<td> 
						<a title="' . Yii::$service->page->translate->__('Dispatch') . '" target="dialog" class="btnEdit" mask="true" drawable="true" width="800" height="300" href="'.$this->_dispatchUrl.'?'.$this->_primaryKey.'='.$one[$this->_primaryKey].'" ><i class="fa fa-edit"></i></a>
