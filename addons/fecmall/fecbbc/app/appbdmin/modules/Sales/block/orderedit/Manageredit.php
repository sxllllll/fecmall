...
class Manageredit extends \yii\base\BaseObject
{
    ...
    public function getLastData()
    {
        ...
        return [
            ...
            'saveUrl' 	    => Yii::$service->url->getUrl('sales/orderedit/managereditsave'),
        ];
    }
        ...
        try {    
            if (is_array($editForm) && $orderModel['order_id']) {
                ...
                if (is_array($products) && !empty($products)) {
                    foreach ($products as $item_id=>$price) {
                        $items[] = [
                            'item_id' => $item_id,
                            'price' => $price,
                        ];
                ...
                if (!Yii::$service->order->item->updatePaymentPendingOrderPrice($orderModel, $items)) {
                    $errors = Yii::$service->helper->errors->get(',');
                    $errors = $errors ? $errors : 'update order price fail';
