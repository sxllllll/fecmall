...
namespace fecbbc\app\appbdmin\modules\Sales\block\orderreceived;
...
class Manager extends AppbdminbaseBlock implements AppbdminbaseBlockInterface
{
    ...
    { 
         ...
         * edit data url
         */
        $this->_editUrl = CUrl::getUrl('sales/orderreceived/manageredit');
        /*
         * delete data url 
         */
        $this->_exportExcelUrl = CUrl::getUrl('sales/orderreceived/managerexport');
    ...
    { 
        $orderStatus = [
            Yii::$service->order->status_received => Yii::$service->page->translate->__(Yii::$service->order->status_received),
        ];
        $data = [
            [    // 字符串类型
                'type' => 'inputtext',
    ...
    public function getTableTbody()
    {
        ...
        $where[] = ['in', 'order_status', Yii::$service->order->info->orderStatusReceivedOrderArr];
        $where[] = ['in', 'order_operate_status', Yii::$service->order->info->orderOperateStatusReceivedOrderArr];
        $filter = [
