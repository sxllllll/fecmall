...
class Manager extends AppbdminbaseBlock implements AppbdminbaseBlockInterface
{
    ...
    { 
         ...
         * delete data url 
         */
        //$this->_exportExcelUrl = CUrl::getUrl('sales/returnwaiting/managerexport');
        $this->_editReturnCostUrl = CUrl::getUrl('sales/returnwaiting/managereditreturncost');
        /*
         * delete data url
         */
        $this->_auditAcceptUrl = CUrl::getUrl('sales/returnwaiting/manageraccept');
        
        $this->_auditRefuseUrl = CUrl::getUrl('sales/returnwaiting/managerrefuse');
        /*
    ...
    { 
        $returnStatus = [
            Yii::$service->order->afterSale->after_sale_status_return_request => Yii::$service->page->translate->__(Yii::$service->order->afterSale->after_sale_status_return_request),
            Yii::$service->order->afterSale->after_sale_status_return_accept => Yii::$service->page->translate->__(Yii::$service->order->afterSale->after_sale_status_return_accept),
            Yii::$service->order->afterSale->after_sale_status_return_refuse => Yii::$service->page->translate->__(Yii::$service->order->afterSale->after_sale_status_return_refuse),
        ];
        $data = [
    ...
    public function getEditBar()
    {
        ...
        */
        return '<ul class="toolBar">
					<li><a csrfName="' .CRequest::getCsrfName(). '" csrfVal="' .CRequest::getCsrfValue(). '" title="' . Yii::$service->page->translate->__('Are you sure audit return request accept in bulk') . '?" target="selectedTodo" rel="'.$this->_primaryKey.'s" postType="string" href="'.$this->_auditAcceptUrl.'" class="edit"><span>' . Yii::$service->page->translate->__('Bulk Audit Accept') . '</span></a></li>
                    <li><a csrfName="' .CRequest::getCsrfName(). '" csrfVal="' .CRequest::getCsrfValue(). '" title="' . Yii::$service->page->translate->__('Are you sure audit return request refuse in bulk') . '?" target="selectedTodo" rel="'.$this->_primaryKey.'s" postType="string" href="'.$this->_auditRefuseUrl.'" class="edit"><span>' . Yii::$service->page->translate->__('Bulk Audit Refuse') . '</span></a></li>
				</ul>';
    }
    ...
    public function getTableTbodyHtml($data)
    {
        ...
        foreach ($data as $one) {
            ...
            $str .= '<td> 
						<a title="' . Yii::$service->page->translate->__('Edit Return Cost') . '" target="dialog" class="btnEdit" mask="true" drawable="true" width="1200" height="680" href="'.$this->_editReturnCostUrl.'?'.$this->_primaryKey.'='.$one[$this->_primaryKey].'" ><i class="fa fa-edit"></i></a>
