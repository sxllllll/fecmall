            ...
            <div class="detail-info ">
                <div class="status">
                    ...
                    <p>
                        ...
                        <em><?=  date('Y-m-d H:i:s', $orderInfo['created_at']) ?></em>
                    </p>
                ...
                <div class="pay-mode">
                    ...
                    <div class="content">
                        <p><?= Yii::$service->page->translate->__('Payment Method') ?>：<?= $orderInfo['payment_method'] ?></p>
                        <p><?= Yii::$service->page->translate->__('Shipping Method') ?>：<?= $shippingLabel ?></p>
                        <?php if ($orderInfo['tracking_company']) : ?>
                            <p><?= Yii::$service->page->translate->__('Shipping Company') ?>：<?= Yii::$service->delivery->kdiniao->getDeliveryShipperName($orderInfo['tracking_company']) ?></p>
                        <?php endif; ?>
                        <?php if ($orderInfo['tracking_number']) : ?>
                            <p><?= Yii::$service->page->translate->__('Tracking No') ?>：<?= $orderInfo['tracking_number'] ?></p>
                        <?php endif; ?>
                    </div>
                </div>
                <div class="good-list">
                                        ...
                                        <tr>
                                            ...
                                            <td>
                                                <div class="p-btns" style="line-height:20px;">
                                                    <?php if ($bInfo['can_review'] && Yii::$service->product->review->hasReviewed($product['is_reviewed'])): ?>
                                                       <a href="javascript:void(0)" class="mt10 btn-again" target="_blank">
                                                            <?= Yii::$service->page->translate->__('Reviewed'); ?>
                                                        </a><br>
                                                    <?php elseif ($bInfo['can_review']): ?>
                                                        <a href="<?= Yii::$service->url->getUrl('catalog/reviewproduct/add', ['order_id' => $orderInfo['order_id'],   'item_id' => $product['item_id'], 'product_id' => $product['product_id']]) ?>" class="mt10 btn-again" target="_blank">
                                                            <?= Yii::$service->page->translate->__('To Review'); ?>
                                                        </a><br>
                                                    <?php endif; ?>
                                                    <a href="<?=  Yii::$service->url->getUrl($product['redirect_url']) ; ?>" class="mt10 btn-again" target="_blank">
