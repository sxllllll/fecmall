...
namespace fecbbc\app\apphtml5\modules\Checkout\block\cart;
                ...
                if (is_array($cartProducts) && !empty($cartProducts)) {
                    foreach ($cartProducts as $k=>$product_one) {
                        ...
                        if (isset($product_one['product_image']['main']['image'])) {
                            $cart_info['products'][$bdmin_user_id][$k]['image'] = $product_one['product_image']['main']['image'];
                        }
                        // 产品的url
                        $cart_info['products'][$bdmin_user_id][$k]['url'] = Yii::$service->url->getUrl($product_one['product_url']);
                        ...
                        if ($custom_option_image) {
                            $cart_info['products'][$bdmin_user_id][$k]['image'] = $custom_option_image;
                        }
                    }
                }
                $cart_info['bdmin'] = Yii::$service->bdminUser->bdminUser->getIdAndPersonArrByIds($bdmin_user_ids);
            }
        }
