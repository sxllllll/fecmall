...
class Index extends \fecshop\app\appserver\modules\Checkout\block\onepage\Index
{
    ...
    public function getLastData()
    {
        ...
        if (!$this->_default_address) {
            $code = Yii::$service->helper->appserver->customer_default_address_is_empty;
            $data = [];
       ...
       if (empty($cartInfo) || !is_array($cartInfo)) {
        ...
        $data = [
            ...
            'default_address'    => $this->_default_address,
   ...
   /**
    ...
    public function getCartOrderInfo()
    {
        ...
        if (!$this->_cart_info) {
            ...
            $postCartCoupon = Yii::$app->request->post('cart_coupon');
            $postShippingMethod = $postShippingMethod ? json_decode($postShippingMethod, true) : '';
            $postCartCoupon = $postCartCoupon ? json_decode($postCartCoupon, true) : '';
            
            
            //var_dump($postShippingMethod, $postCartCoupon);exit;
