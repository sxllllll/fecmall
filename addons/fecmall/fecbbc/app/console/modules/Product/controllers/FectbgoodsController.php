...
namespace fecbbc\app\console\modules\Product\controllers;
 ...
 * @since 1.0
 */
class FectbgoodsController extends Controller
{
     ...
     */
    public function actionInitprocess($pageNum = 1)
    {
        ...
        foreach ($coll as $product) {
            Yii::$service->product->initProductDeputy($product);
        }
        
    }
    // 得到个数
    public function actionInitcount()
    {
        $count = Yii::$service->product->collCount();
        echo $count ;
    }
    // 得到个数
    public function actionInitpagenum()
    {
        $count = Yii::$service->product->collCount();
