...
class Cart extends \fecshop\services\Cart
{
     ...
     */
    public function getCartInfo2($activeProduct = true)
    {
    ...
    {
        return Yii::$service->cart->quote->getCartOrderInfo($postShippingMethod, $postCartCoupon);
    }
    ...
    {
        $status = Yii::$service->cart->quoteItem->selectChoseItem($checked, $items);
        if (!$status) {
            return false;
        }
        Yii::$service->cart->quote->computeCartInfo();
    ...
    public function addProductToBuyNowCart($item)
    {
        ...
        if (!Yii::$service->cart->quoteItem->addBuyNowItem($item, $product)) {
            return false;
        }
    ...
    public function addProductToCart($item)
    {
        ...
        // 查看产品是否有bdmin_user_id,以及是否是active
        $bdmin_user_id = $product['bdmin_user_id'];
        if (!$bdmin_user_id) {
            Yii::$service->helper->errors->add('product bdmin_user_id is not exist');

            return false;
        } 
        if (!Yii::$service->bdminUser->bdminUser->isActiveBdminUser($bdmin_user_id)) {
            Yii::$service->helper->errors->add('product bdmin_user_id is not active');
