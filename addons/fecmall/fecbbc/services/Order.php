...
class Order extends \fecshop\services\Order
{
    ...
    protected $_orderModelName = '\fecbbc\models\mysqldb\Order';
     ...
     * 将订单所有的状态，组合成一个数组，进行返回。
     */
    public function getOperateStatusArr()
    {
        return [
     ...
     * 将订单所有的状态，组合成一个数组，进行返回。
     */
    public function getSelectAllStatusArr()
    {
        return [
     ...
     * 将订单所有的状态，组合成一个数组，进行返回。
     */
    public function getSelectOperateStatusArr()
    {
        return [];
     ...
     *               然后进行一系列的处理，返回订单数组数据。
     */
    public function getOrdersInfoByPayNo($payNo)
    {
     ...
     */
    public function updatePaymentMethod($increment_id, $payment_method, $paramIsVerifyed = false)
    {
     ...
     * @param $payment_method | Array 支付方式、['bdmin_user_id'  => 'shipping_method']
     ...
     */
    public function generateOrderByCart2($address, $shipping_methods, $cart_coupon, $payment_method, $clearCart = true, $token = '', $order_remark = '')
    {
        ...
        $state          = $address['state'];
        $cartInfo       = Yii::$service->cart->getCartOrderInfo($shipping_methods, $cart_coupon);
        // 检查cartInfo中是否存在产品
        ...
        */
        ...
        $orderPaymentBaseTotal = 0;
        $orderPaymentCurrTotal = 0;
        foreach($cartInfo['cart_info'] as $bdmin_user_id => $bdmin_cart_info) {
            ...
            $orderPaymentBaseTotal          += $bdmin_cart_info['base_grand_total'];
            $orderPaymentCurrTotal           += $bdmin_cart_info['grand_total'];
            ...
            */
            ...
            $couponCode = isset($bdmin_cart_info['coupon_selected']['code']) ? $bdmin_cart_info['coupon_selected']['code'] : '' ;
            ...
            $myOrder['customer_address_area']   = $address['area'];
            ...
            $myOrder['coupon_code']             = $couponCode;
            ...
            // 将使用的优惠券，进行状态更改。
            Yii::$service->coupon->customer->useCoupon($customer_id, $couponCode);
        ...
        // 生成的所有订单的总额。
        $this->orderPaymentInfo['baseTotal'] = Yii::$service->helper->format->numberFormat($orderPaymentBaseTotal);
        $this->orderPaymentInfo['currTotal']  = Yii::$service->helper->format->numberFormat($orderPaymentCurrTotal);
    ...
    function generatePaymentNo()
    {
        ...
        $uuidStr = substr($uuidStr,9, 24);
        return 'pay_no_' . $uuidStr;
    }
    ...
    */
    ...
    protected $_sessionTradeNo;
    /**
     * @param $increment_id | String ,order订单号
     *  订单交易号，里面可以是订单号，也可能是支付号
     */
    public function setSessionTradeNo($trade_no)
    {
        if (Yii::$service->helper->isApiApp()) {
            $this->_sessionTradeNo = $trade_no;
        } else {
     ...
     * 从session中取出来订单交易号.
     */
    public function getSessionTradeNo()
    {
        // 如果是api入口
        if (Yii::$service->helper->isApiApp()) {
            
            return $this->_sessionTradeNo;
        } else {
     ...
     * 从session中销毁订单号.
     */
    public function removeSessionTradeNo()
    {
     ...
     * 通过 order_id 生成订单号。
     */
    public function generateIncrementIdByOrderId($order_id)
    {
     ...
     * 下面的数据是为了后台的订单统计
     */
    public function getBdminUserPreMonthOrder($days, $bdminUserId)
    {
        // 得到一个月前的时间戳
        $preMonthTime = strtotime("-$days days");
        $filter = [
    ...
    // 更新订单的评论状态
    public function updateOrderReviewStatus($orderId, $orderItemId)
    {
        // 更新订单产品评论状态
        $orderItemModel = Yii::$service->order->item->getByPrimaryKey($orderItemId);
        if (!$orderItemModel['item_id']) {
            return false;
        }
        if ($orderItemModel->is_reviewed == Yii::$service->product->review->isReviewed) {
            Yii::$service->helper->errors->add('order product has reviewed');
            
            return false;
        }
        $orderItemModel->is_reviewed = Yii::$service->product->review->isReviewed;
        if (!$orderItemModel->save()) {
            return false;
        }
        
        $orderReviewStatus = true;
        $orderItems = Yii::$service->order->item->getByOrderId($orderId, true);
        if (!is_array($orderItems)) {
            return false;
        }
        
        foreach ($orderItems as $orderItem) {
            if (!Yii::$service->product->review->hasReviewed($orderItem['is_reviewed'])) {
                $orderReviewStatus = false;
                break;
            }
        }
        
        if ($orderReviewStatus ) {
            $order = Yii::$service->order->getByPrimaryKey($orderId);
            $order->is_reviewed = Yii::$service->product->review->isReviewed;
