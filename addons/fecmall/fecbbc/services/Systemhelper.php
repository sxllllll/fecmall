 ...
 * @since 1.0
 */
class Systemhelper extends \fecshop\services\Systemhelper
{
     ...
     */
    public function getRecentBdminStatisticsInfo($bdminUserId)
    {
        // 1.1总量：产品Sku总数
        $productFilter = [
            'select' => ['sku'],
        ...
        */
        ...
        $filter = [
            'select' => ['increment_id'],
            'where'			=> [
                ['in', 'order_status', Yii::$service->order->info->orderStatusIsPaymentCompleteArr],
                ['in', 'order_operate_status', Yii::$service->order->info->orderOperateStatusIsPaymentCompleteArr],
                ['bdmin_user_id' => $bdminUserId]
            ],
            'asArray' => true,
        ];
    ...
    public function getRecentStatisticsInfo()
    {
        ...
        // 2.1昨日/今日 订单总销售额，总数，下单用户数
        $yest0Time = strtotime(date("Y-m-d",strtotime("-1 day")));   // 昨日0点
        $today0Time = strtotime(date("Y-m-d",time()));   // 今日0点
        $filter = [
            'select' => ['created_at', 'customer_id', 'order_status', 'base_grand_total' ],
            'fetchAll' => true,
            'where'			=> [
