 ...
 * @see \fecshop\services\cart\Quote::setCartId()
 *
...
class Quote extends \fecshop\services\cart\Quote
{
     ...
     * ]
     */
    public function getCartOrderInfo($postShippingMethod, $postCartCoupon)
    {
        $cart_id = $this->getCartId();
        if (!$cart_id) {
            return false;
        ...
        $all_base_total = 0;
        $all_shipping_cost = 0;
        $all_shipping_base_cost = 0;
        $all_product_total = 0;
        $all_product_base_total = 0;
        $all_discount = 0;
        $all_base_discount = 0;
        // 计算优惠券
        ...
        foreach ($cart_product_info as $bdmin_user_id => $cart_one) {
            $items = $cart_one['products'];
            if (is_array($items)) {
                foreach ($items as $item) {
                    $couponCartItems[$bdmin_user_id][] = [
                        'sku' => $item['sku'], 
                        'product_id' => $item['product_id'],
                        'base_row_total'  => $item['base_product_row_price'],
                    ];
                }
            }
        }
        $cartCoupons = Yii::$service->coupon->customer->getCartCouponList($customerId, $couponCartItems, $postCartCoupon);
        $availableCoupons = [];
        $unavailableCoupons = [];
        $selectedCostCoupon = [];
        if (isset($cartCoupons['availableCoupons']) && is_array($cartCoupons['availableCoupons'])) {
            $availableCoupons = $cartCoupons['availableCoupons'];
        }
        if (isset($cartCoupons['selectedCoupon']) && is_array($cartCoupons['selectedCoupon'])) {
            $selectedCostCoupon = $cartCoupons['selectedCoupon'];
        }
        if (isset($cartCoupons['unavailableCoupons']) && is_array($cartCoupons['unavailableCoupons'])) {
            $unavailableCoupons = $cartCoupons['unavailableCoupons'];
        }
        ...
        foreach ($cart_product_info as $bdmin_user_id => $cart_one) {
            ...
            $shipping_method_index = $shippingCost['shipping_method_index'];
            $shipping_method_label = $shippingCost['shipping_method_label'];
            $shippingMethods = $shippingCost['shippings'];
            // 优惠券优惠金额
            $baseDiscountCost = 0;
            $currDiscountCost = 0;
            $selectedCoupon = isset($selectedCostCoupon[$bdmin_user_id]) ? $selectedCostCoupon[$bdmin_user_id] : '';
            
            if (isset($selectedCoupon['discount_cost']) && $selectedCoupon['discount_cost']) {
                $baseDiscountCost = (float)$selectedCoupon['discount_cost'];
                $currDiscountCost = (float)Yii::$service->page->currency->getCurrentCurrencyPrice($baseDiscountCost);
            }
            //var_dump($baseDiscountCost, $currDiscountCost);
            // 当前经销商的总额
            ...
            $cart_product_info[$bdmin_user_id]['shipping_method_index'] = $shipping_method_index;
            ...
            $cart_product_info[$bdmin_user_id]['coupon_selected'] = $selectedCoupon;
            $cart_product_info[$bdmin_user_id]['coupon_available'] = isset($availableCoupons[$bdmin_user_id]) ? $availableCoupons[$bdmin_user_id] : [];
            $cart_product_info[$bdmin_user_id]['coupon_unavailable'] = isset($unavailableCoupons[$bdmin_user_id]) ? $unavailableCoupons[$bdmin_user_id] : [];
            // var_dump($cart_product_info[$bdmin_user_id]['coupon_unavailable']);
            
            $all_count += $product_qty_total;
            $all_total += $curr_grand_total;
            $all_base_total += $base_grand_total;
            
            $all_discount += $currDiscountCost;
            $all_base_discount += $baseDiscountCost;
            
            $all_shipping_cost += $currShippingCost;
            $all_shipping_base_cost += $baseShippingCost;
            $all_product_total += $product_total;
            $all_product_base_total += $base_product_total;
        }
        ...
        return [
            ...
            'all_base_discount' => $all_base_discount,
            
            'all_product_total' => $all_product_total,
            'all_product_base_total' => $all_product_base_total,
            
            'all_shipping_cost' => $all_shipping_cost,
            'all_shipping_base_cost' => $all_shipping_base_cost,
