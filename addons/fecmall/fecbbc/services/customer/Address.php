...
class Address extends \fecshop\services\customer\Address
{
    ...
    protected $_addressModelName = '\fecbbc\models\mysqldb\customer\Address';
     ...
     */
    public function getAddressByIdAndCustomerId($address_id, $customer_id)
    {
        $primaryKey = $this->getPrimaryKey();
     ...
     */
    /*
    public function currentAddressList()
    {
        $arr = [];
        if (!Yii::$app->user->isGuest) {
            $identity = Yii::$app->user->identity;
            ...
            if ($customer_id) {
                ...
                if (is_array($coll['coll']) && !empty($coll['coll'])) {
                    ...
                    if (!$ii) {
                        // 如果没有默认的地址，则取第一个当默认
                        foreach ($arr as $k=>$v) {
                            $arr[$k]['is_default'] = $this->is_default;
                            break;
                        }
                    }
                }
    ...
    */
    ...
    public function save($one)
    {
        ...
        if ($one['is_default'] === true || $one['is_default'] === false) {
            $one['is_default'] = $one['is_default'] == true ? $this->is_default : $this->is_not_default;
        }
        if ($one['is_default'] === 'true' || $one['is_default'] === 'false') {
            $one['is_default'] = $one['is_default'] == 'true' ? $this->is_default : $this->is_not_default;
        }
     ...
     */
    public function remove($ids, $customer_id)
    {
        if (!$ids) {
        ...
        $addressOne = $this->_addressModel->find()->asArray()
                    ->where(['customer_id' => $customer_id, 'is_default' => $this->is_default])
                    ->one();
        if (!$addressOne['address_id']) {
            ...
            if ($assOne['address_id']) {
                $assOne->is_default = $this->is_default;
