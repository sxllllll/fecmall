...
class Info extends \fecshop\services\Service
{
    ...
    // 订单流程状态：订单已完成支付   ///////////
    public $orderStatusIsPaymentCompleteArr;
    // 订单操作状态：订单已完成支付   ///////////
    public $orderOperateStatusIsPaymentCompleteArr;
    
    // 订单流程状态：订单等待发货
    public $orderStatusWaintingDispatchArr;
    // 订单操作状态：订单等待发货
    public $orderOperateStatusWaintingDispatchArr;
    // 订单流程状态：订单等待收货
    public $orderStatusWaintingReceiveArr;
    // 订单操作状态：订单等待收货
    public $orderOperateStatusWaintingReceiveArr;
    // 订单流程状态：订单已完成
    public $orderStatusCompleteArr;
    // 订单操作状态：订单已完成
    public $orderOperateStatusCompleteArr;
    // 订单操作状态：订单已取消
    public $orderOperateStatusCancelArr;
    ...
    // 订单流程状态：允许用户进行【订单物流信息查询】的状态数组
    public $orderStatusCanQueryTrackingArr;
    // 订单操作状态：允许用户进行【订单物流信息查询】的状态数组
    public $orderOperateStatusCanQueryTrackingArr;
    
    // 订单流程状态：允许用户进行【订单再次购买】的状态数组
    public $orderStatusCanReOrderArr;
    // 订单操作状态：允许用户进行【订单再次购买】的状态数组
    public $orderOperateStatusCanReOrderArr;
    
    // 订单流程状态：允许用户进行【订单产品评论】的状态数组
    public $orderStatusCanReviewArr;
    // 订单操作状态：允许用户进行【订单产品评论】的状态数组
    public $orderOperateStatusCanReviewArr;
    ...
    // 订单售后退货状态：用户可以提交或者查看物流信息的状态
    public $afterSaleReturnStatusCanShowShippingArr;
    ...
    public function init()
    {
        ...
        $this->orderStatusWaintingDispatchArr = [
            Yii::$service->order->payment_status_confirmed,
            Yii::$service->order->status_processing,
        ];
        
        $this->orderOperateStatusWaintingDispatchArr = [
             Yii::$service->order->operate_status_normal,
        ];
        
        $this->orderStatusWaintingReceiveArr = [
            Yii::$service->order->status_dispatched,
        ];
        
        $this->orderOperateStatusWaintingReceiveArr = [
             Yii::$service->order->operate_status_normal,
        ];
        
        $this->orderStatusCompleteArr = [
            Yii::$service->order->status_received,
            Yii::$service->order->status_completed,
        ];
        
        $this->orderOperateStatusCompleteArr = [
            Yii::$service->order->operate_status_normal,
        ];
        
        $this->orderOperateStatusCancelArr = [
            Yii::$service->order->operate_status_canceled,
        ];
        
        $this->orderStatusIsPaymentCompleteArr = [
            Yii::$service->order->payment_no_need_status_confirmed,
            Yii::$service->order->payment_status_confirmed,
            Yii::$service->order->status_audit_fail,
            Yii::$service->order->status_processing,
        ...
        $this->orderOperateStatusIsPaymentCompleteArr = [
            Yii::$service->order->operate_status_normal,
            Yii::$service->order->operate_status_waiting_canceled,
            Yii::$service->order->operate_status_holded,
        ];
        ...
        $this->orderStatusCanQueryTrackingArr = [
            Yii::$service->order->status_dispatched,
            Yii::$service->order->status_received,
        ];
        
        $this->orderOperateStatusCanQueryTrackingArr = [
            Yii::$service->order->operate_status_normal,
        ]; 
        
        
        $this->orderStatusCanReOrderArr = [
            Yii::$service->order->status_received,
        ];
        
        $this->orderOperateStatusCanReOrderArr = [
            Yii::$service->order->operate_status_normal,
        ]; 
        
        $this->orderStatusCanReviewArr = [
            Yii::$service->order->status_received,
        ];
        
        $this->orderOperateStatusCanReviewArr = [
            Yii::$service->order->operate_status_normal,
        ]; 
        ...
        $this->afterSaleReturnStatusCanShowShippingArr = [
            Yii::$service->order->afterSale->after_sale_status_return_accept,
            ...
            Yii::$service->order->afterSale->after_sale_status_return_refund,
        ];
    ...
    public function isConsoleCanCancelPaymentPendingOrder($orderModel)
    {
     ...
     /**
     * @param $orderModel | Object,  ordermodel
     * @return boolean
     * 通过订单model，判断订单是否已经支付。
     */
    public function isPaymentCompleteOrder($orderModel)
    {
        if (!in_array($orderModel['order_status'],  $this->orderStatusIsPaymentCompleteArr)) {
            
            return false;
        } 
        if (!in_array($orderModel['order_operate_status'],  $this->orderOperateStatusIsPaymentCompleteArr)) {
     ...
     * @param $afterSale | Object,  order after sale model
     * @return boolean
     * 订单售后退货，是否可以查看售后退货shipping或者发货操作的状态
     */
    public function isCustomerCanShowAfterSaleReturndShipping($afterSale)
    {
        $afterSaleStatus = $afterSale['status'];
        if (!in_array($afterSaleStatus,  $this->afterSaleReturnStatusCanShowShippingArr)) {
    ...
    /**
     * @param $order | object or Array, order data
     * @return boolean, 返回订单是否,前端用户查询订单物流追踪信息
     * 【订单物流部分：订单是否可以查询物流追踪信息】，用于在用户账户中心，已发货的订单，查询物流信息
     */
    public function isCustomerCanQueryTracking($order) 
    {
        if (!in_array($order['order_status'],  $this->orderStatusCanQueryTrackingArr)) {
            
            return false;
        } 
        if (!in_array($order['order_operate_status'],  $this->orderOperateStatusCanQueryTrackingArr)) {
    ...
    /**
     * @param $order | object or Array, order data
     * @return boolean, 返回订单是否,前端用户是否可以再次下单（也就是将订单里面的产品全部加入购物车，一般发生在订单完成后，用户进行再次购买操作）
     * 【订单完成后部分：订单是否可以再次购买】
     */
    public function isCustomerCanReOrder($order) 
    {
        if (!in_array($order['order_status'],  $this->orderStatusCanReOrderArr)) {
            
            return false;
        } 
        if (!in_array($order['order_operate_status'],  $this->orderOperateStatusCanReOrderArr)) {
    ...
    /**
     * @param $order | object or Array, order data
     * @return boolean, 返回订单是否,前端用户是否可以评论订单产品
     * 【订单完成后部分：订单是否是否可以评论订单产品】
     */
    public function isCustomerCanReview($order) 
    {
        if (!in_array($order['order_status'],  $this->orderStatusCanReviewArr)) {
            
            return false;
        } 
        
        if (!in_array($order['order_operate_status'],  $this->orderOperateStatusCanReviewArr)) {
            
            return false;
        } 
        if (Yii::$service->product->review->hasReviewed($order['is_reviewed'])) {
