...
class Process extends \fecshop\services\Service
{
    ...
    {
        $this->delayReceiveOrderMaxDays = Yii::$app->store->get('order', 'delay_receive_order_max_days');
        $this->delayReceiveOrderDaysPerTime = Yii::$app->store->get('order', 'delay_receive_order_days_per_time');
        $this->delayReceiveOrderTriggerDays = Yii::$app->store->get('order', 'delay_receive_order_trigger_days');
        $this->orderDefaultMaxRecevieDay = Yii::$app->store->get('order', 'order_default_max_recevie_day');
    }
    ...
    public function cancelPaymentPendingOrder($orderModel)
    {
        ...
        $couponCode = $orderModel['coupon_code'];
        Yii::$service->coupon->customer->revertCoupon($customerId, $couponCode);
     ...
     * @param $order_id | int, 订单id
     * @param $tracking_company | string，货运公司编号
     * @param $tracking_number | string，物流追踪号
     * @return boolean, 
     * 后台bdmin，订单发货
     */
    public function bdminDispatchOrderById($bdmin_user_id, $order_id, $tracking_company, $tracking_number) {
        $orderModel = Yii::$service->order->getByPrimaryKey($order_id);
        if ($bdmin_user_id !=  $orderModel['bdmin_user_id']) {
            Yii::$service->helper->errors->add('you do not have role');
            
            return false;
        }
        if (!$tracking_company) {
            Yii::$service->helper->errors->add('tracking company is empty');
        ...
        if (Yii::$service->order->info->isBdminCanDispatchOrder($orderModel)) {
            if ($this->dispatchOrder($orderModel, $tracking_company, $tracking_number)) {
     ...
     * 后台bdmin，订单发货
     */
    public function bdminDispatchOrderByIncrementId($increment_id, $tracking_company, $tracking_number, $bdmin_user_id) {
        $orderModel = Yii::$service->order->getByIncrementId($increment_id);
    ...
    public function customerDelayReceiveOrderById($order_id, $customer_id='')
    {
        ...
        if (!Yii::$service->order->info->isCustomerCanDelayReceiveOrder($orderModel)) {
            Yii::$service->helper->errors->add('customer can not delay receive order');
     ...
     * 后台bdmin，订单发货
     */
    public function dispatchOrder($orderModel, $tracking_company, $tracking_number) 
    {
        if (!isset($orderModel['increment_id']) || !$orderModel['increment_id']) {
            Yii::$service->helper->errors->add('order model is empty');
        ...
        $updateArr['tracking_company'] = $tracking_company;
