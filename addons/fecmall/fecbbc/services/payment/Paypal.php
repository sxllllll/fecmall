 ...
 * @since 1.0
 */
class Paypal extends \fecshop\services\payment\Paypal
{
    /**
        ...
        // 指定了订单状态
            ...
            if ($payment_status == $this->payment_status_completed) {
                ...
                foreach ($this->ipn_order_models as $orderModel) {
                    ...
                    $updateArr['payment_fee'] = Yii::$service->helper->format->numberFormat($payment_fee * $order_grand_total / $all_total);
   ...
   public function getExpressCheckoutUrl($token)
    ...
    public function getCheckoutPaymentNvpStr($token)
    {
        ...
        foreach($orderInfos as $orderInfo) {
            ...
            $subtotal_s       = Yii::$service->helper->format->numberFormat($orderInfo['subtotal']);
