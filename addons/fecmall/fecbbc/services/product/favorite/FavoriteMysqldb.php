...
namespace fecbbc\services\product\favorite;
 ...
 * @since 1.0
 */
class FavoriteMysqldb extends \fecshop\services\product\favorite\FavoriteMysqldb
{
    protected $_favoriteModelName = '\fecshop\models\mysqldb\product\Favorite';
     ...
     */
    public function addFav($bdmin_user_id, $product_id, $user_id)
    {
        $user_id = (int) $user_id;
    ...
    public function currentUserRemove($favorite_id)
    {
        ...
        $favoritePrimaryKey = $this->getPrimaryKey();
        $one = $this->_favoriteModel->findOne([
            $favoritePrimaryKey        => $favorite_id,
            'user_id'    => $user_id,
        ]);
        if ($one[$favoritePrimaryKey]) {
            $one->delete();
