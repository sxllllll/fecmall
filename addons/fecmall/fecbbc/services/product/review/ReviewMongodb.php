...
namespace fecbbc\services\product\review;
 ...
 * @since 1.0
 */
class ReviewMongodb extends \fecshop\services\product\review\ReviewMongodb
{
     ...
     * 增加评论 前台增加评论调用的函数。
     */
    public function addOrderReview($review_data)
    {
        //$this->initReviewAttr($review_data);
        $model = new $this->_reviewModelName();
        if (isset($review_data[$this->getPrimaryKey()])) {
        ...
        $model['user_id'] = $review_data['customer_id'];
        $model['store'] = Yii::$service->store->currentStore;
        $model['lang_code'] = Yii::$service->store->currentLangCode;
        $model['review_date'] = time();
        $model['ip'] = \fec\helpers\CFunc::get_real_ip();
        
        $model['product_spu'] = $review_data['spu'];
        $model['product_sku'] = $review_data['sku'];
        $model['bdmin_user_id'] = $review_data['bdmin_user_id'];
        $model['product_id'] = $review_data['product_id'];
        $model['rate_star'] = $review_data['rate_star'];
        $model['name'] = $review_data['customer_name'];
        $model['summary'] = '';
        $model['review_content'] = $review_data['review_content'];
